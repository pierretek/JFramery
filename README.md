# JFramery &nbsp;<img src="assets/jframery.png" width="40" style="vertical-align: middle;" />
Three simple java minigames using jframes<br>
(JFrames + Tomfoolery = JFramery)
 
## Details 
- Main class is in `src/jframery/JFramery.java`.
- You can choose between three different minigames
- Highscores are saved persistently in `scores.txt`
- Scores are tied to player names and can be viewed or deleted

## Requirements 
- Java JDK version 14 or higher
- *Netbeans IDE (if you wanna build it yourself)*

## How to Run
1. Download the `JFramery Playable` zip file from releases
2. Extract it and open the folder in a terminal
3. Run ``java -jar JFramery.jar`` to run the jar file
4. Enjoy!

*If you want to build this project yourself, you can clone this repo and import it into Netbeans as a project.*

## Minigame Overview 

This program has a total of three playable minigames: <br>
⇒ **Window Hunter** <br>
⇒ **Window Chaser** <br>
⇒ **Window Survivor** <br>

#### Window Hunter
- A bunch of random windows will pop up on your screen
- Close the newest window before another appears
- *Tip: Ignore previous windows and focus on closing the newest one*

#### Window Chaser
- A small window will spawn on the screen
- Chase the window across your screen and try to close it!
- *Tip: The window runs away From your cursor*

#### Window Survivor
- A red window will follow your cursor, and randomly placed windows will fall accross the screen
- Navigate the red window between the falling cascade of obstacles!
- *Tip: Avoid the white windows at all costs*
> [!WARNING]
> This game might get a little laggy depending on your computer

---

###### thanks for reading
